...
function OffspringMonitoringList() {
    const [IsOffSpringMonitoringModalOpen, setIsOffSpringMonitoringModalOpen] = useState(false);
    ...
    useEffect(() => {
        const fetchMonitoringRecords = async () => {
            try {
                ...
                const uniqueMunicipalities = [...new Set(response.data.map(record => record.municipality).filter(Boolean))];
    ...
    const handleEditStatus = async (recordId, newStatus) => {
        try {
            await axiosInstance.put(`/api/offspring-monitoring/${recordId}`, { formStatus: newStatus });
            setMonitoringRecords(monitoringRecords.map(record =>
                record._id === recordId ? { ...record, formStatus: newStatus } : record
            ));
    ...
    const filteredRecords = monitoringRecords.filter(record => {
        ...
        const matchesSearch = record.entries.some(entry =>
            entry.name.toLowerCase().includes(searchTerm) ||
            entry.species.toLowerCase().includes(searchTerm)
        );

        const matchesMunicipality = !filters.municipality || record.municipality === filters.municipality;
        const matchesStatus = !filters.status || record.formStatus === filters.status;
    ...
    return (
        <div className="p-6">
            ...
            {filteredRecords.length === 0 ? (
                <p className="text-center py-4">No records found matching the filters.</p>
            ) : (
                        ...
                        <tbody>
                            {filteredRecords.map((record, index) => (
                                <tr key={record._id} className="hover:bg-gray-50">
                                    ...
                                    <td className="border border-gray-300 p-4">{record.municipality}</td>
                                    <td className="border border-gray-300 p-4">{formatDate(record.dateReported)}</td>
                                    <td className="border border-gray-300 p-4">{record.formStatus}</td>
            ...
            {selectedRecord && (
                <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
                    <h3 className="text-xl font-bold mb-4">Edit Status for {selectedRecord.municipality} {formatDate(selectedRecord.dateReported)}</h3>
                    <div className="mb-4">
                        <label htmlFor="status" className="block text-sm font-medium mb-2">Status:</label>
                        <select
                            id="status"
                            value={selectedRecord.formStatus}
                            onChange={(e) => setSelectedRecord(prev => ({ ...prev, formStatus: e.target.value }))}
                            className="p-2 border rounded w-full"
                        >
                    ...
                    <div className="flex justify-end">
                        <button
                            onClick={() => handleEditStatus(selectedRecord._id, selectedRecord.formStatus)}
                        ...
                        <table className="min-w-full border-collapse border border-gray-300">
                            <thead>
                                <tr className="bg-gray-800 text-white">
                                    ...
                                    <th className="border border-gray-300 p-4">Date of Monitoring</th>
                            ...
                            <tbody>
                                {selectedEntries.map((entry, index) => (
                                    <tr key={entry._id} className="hover:bg-gray-50">
                                        ...
                                        <td className="border border-gray-300 p-4">{new Date(entry.dateOfMonitoring).toLocaleDateString()}</td>
                                        <td className="border border-gray-300 p-4">{entry.barangay}</td>
                                        <td className="border border-gray-300 p-4">{entry.species}</td>
                                        <td className="border border-gray-300 p-4">{entry.aiTech}</td>
                                        <td className="border border-gray-300 p-4">{new Date(entry.calvingDate).toLocaleDateString()}</td>
            ...
            {IsOffSpringMonitoringModalOpen && (
                <Modal isOpen={IsOffSpringMonitoringModalOpen} onClose={() => setIsOffSpringMonitoringModalOpen(false)}>
                    <OffspringMonitoring />
                </Modal>
            )}
        </div>
