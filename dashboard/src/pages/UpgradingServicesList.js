...
function UpgradingServicesList() {
    ...
    const fetchServices = async () => {
      try {
        ...
        const uniqueMunicipalities = [
          ...new Set(
            response.data.map((service) => service.municipality).filter(Boolean)
          ),
        ];
  ...
  const handleEditStatus = async (serviceId, newStatus) => {
    try {
      await axiosInstance.put(`/api/upgrading-services/${serviceId}`, {
        formStatus: newStatus,
      });
      setServices(
        services.map((service) =>
          service._id === serviceId
            ? { ...service, formStatus: newStatus }
            : service
        )
      );
  ...
  const filteredServices = services.filter((service) => {
    ...
    const matchesSearch = service.entries.some(
      (entry) =>
    ...
    const matchesMunicipality =
      !filters.municipality || service.municipality === filters.municipality;
    const matchesStatus =
      !filters.status || service.formStatus === filters.status;
  ...
  return (
    <div className="p-6">
            ...
            <tbody>
              {filteredServices.map((service, index) => (
                <tr key={service._id} className="hover:bg-gray-50">
                  ...
                  <td className="border border-gray-300 p-4">
                    {formatDate(service.dateReported)}
                  </td>
                  <td className="border border-gray-300 p-4">
                    {service.formStatus}
                  </td>
      ...
      {selectedService && (
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
          <h3 className="text-xl font-bold mb-4">
            Edit Status for {selectedService.municipality}{" "}
            {formatDate(selectedService.dateReported)}
          </h3>
          <div className="mb-4">
              ...
              <tbody>
                {selectedEntries.map((entry, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="border border-gray-300 p-4">
                      {entry.clientInfo.gender}
                    </td>
                    <td className="border border-gray-300 p-4">
                      {new Date(entry.clientInfo.birthday).toLocaleDateString()}
                    </td>
                    <td className="border border-gray-300 p-4">
                      {entry.clientInfo.contactNo}
                    </td>
                    <td className="border border-gray-300 p-4">
                      {entry.animalInfo.species}
                    </td>
                    <td className="border border-gray-300 p-4">
                      {entry.animalInfo.breed}
                    </td>
                    <td className="border border-gray-300 p-4">
                      {entry.animalInfo.sex}
                    </td>
                    <td className="border border-gray-300 p-4">
                      {entry.animalInfo.age}
                    </td>
                    <td className="border border-gray-300 p-4">
                      {entry.animalInfo.color}
                    </td>
                    <td className="border border-gray-300 p-4">
                      {entry.animalInfo.estrus}
                    </td>
                    <td className="border border-gray-300 p-4">
                      {entry.sireCodeNum}
                    </td>
      ...
      {isUpgradingModalOpen && (
        <Modal
          isOpen={isUpgradingModalOpen}
          onClose={() => setIsUpgradingModalOpen(false)}
        >
          <UpgradingServices />
        </Modal>
      )}
