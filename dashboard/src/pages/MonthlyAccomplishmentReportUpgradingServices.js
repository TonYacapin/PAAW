...
const MonthlyAccomplishmentReportUpgradingServices = () => {
  ...
  useEffect(() => {
    const filtered = data.filter((item) => {
      ...
      const matchesActivity = item.entries.some((entry) =>
        entry.activity.toLowerCase().includes(filter.activity.toLowerCase())
      );
      ...
      const matchesMonth = filter.month
        ? new Date(item.dateReported).getMonth() + 1 === parseInt(filter.month)
        : true;
      const matchesYear = filter.year
        ? new Date(item.dateReported).getFullYear() === parseInt(filter.year)
        : true;
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">
        Accomplishment Report Upgrading Services
      </h1>
        ...
        <>
          ...
          <div className="bg-white p-4 mb-6 shadow-md rounded-md">
            <p className="font-semibold text-black">
              Total Entries:{" "}
              {filteredData.reduce((acc, item) => acc + item.entries.length, 0)}
            </p>
            <p className="font-semibold text-black">
