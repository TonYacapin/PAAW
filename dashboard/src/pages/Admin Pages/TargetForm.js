...
const TargetForm = ({ onClose, target }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = target ? `${process.env.REACT_APP_API_BASE_URL}/api/targets/${target._id}` : `${process.env.REACT_APP_API_BASE_URL}/api/targets`;
    ...
    return (
        <div>
            <h2 className="text-xl font-bold mb-4">{target ? 'Edit Target' : 'Add Target'}</h2>
