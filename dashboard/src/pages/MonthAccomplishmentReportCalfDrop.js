...
const MonthAccomplishmentReportCalfDrop = () => {
  ...
  useEffect(() => {
    const filtered = data.filter((item) => {
      const matchesMunicipality = item.municipality
        .toLowerCase()
        .includes(filter.municipality.toLowerCase());
      const matchesTechnicianName = item.technicianName
        .toLowerCase()
        .includes(filter.technicianName.toLowerCase());

      // Check if month and year match
      const matchesMonth = filter.month
        ? new Date(item.dateSubmitted).getMonth() + 1 === parseInt(filter.month)
        : true;
      const matchesYear = filter.year
        ? new Date(item.dateSubmitted).getFullYear() === parseInt(filter.year)
        : true;

      return (
        matchesMunicipality &&
        matchesTechnicianName &&
        matchesMonth &&
        matchesYear
      );
    });
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Calf Drop Quarterly Report</h1>
        ...
        <>
          ...
          <div className="bg-white p-4 mb-6 shadow-md rounded-md">
            <p className="font-semibold text-black">
              Total Entries:{" "}
              {filteredData.reduce(
                (acc, item) => acc + item.animalEntries.length,
                0
              )}
            </p>
            <p className="font-semibold text-black">
              Total Municipalities: {filteredData.length}
            </p>
          </div>
            ...
            <table className="min-w-full bg-white rounded-md shadow-md">
              ...
              <tbody className="text-black text-sm">
                {filteredData.map(
                  (item, index) =>
                    item.animalEntries &&
                    Array.isArray(item.animalEntries) &&
                    item.animalEntries.map((entry, entryIndex) => (
                      <tr
                        key={`${item._id}-${entryIndex}`}
                        className="border-b border-black-200 hover:bg-black-100"
                      >
                        ...
                        <td className="py-3 px-6">
                          {new Date(entry.dateCalved).toLocaleDateString()}
                        </td>
                      </tr>
                    ))
                )}
