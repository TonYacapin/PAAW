...
function TechnicianQuarterlyReportFormList() {
  ...
  const handleEditStatus = async (reportId, newStatus) => {
    try {
      await axiosInstance.put(`/api/technician-quarterly/${reportId}`, {
        formStatus: newStatus,
      });
      setReports(
        reports.map((report) =>
          report._id === reportId
            ? { ...report, formStatus: newStatus }
            : report
        )
      );
  ...
  const filteredReports = reports.filter((report) => {
    ...
    const matchesSearch = report.technicianName
      .toLowerCase()
      .includes(searchTerm);

    const matchesStatus =
      !filters.status || report.formStatus === filters.status;
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-6">
        Technician Quarterly Report List
      </h2>
      ...
      {selectedReport && (
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
          <h3 className="text-xl font-bold mb-4">
            Edit Status for {selectedReport.technicianName}
          </h3>
          <div className="mb-4">
          ...
          <div className="flex justify-end space-x-2">
            <button
              onClick={() =>
                handleEditStatus(selectedReport._id, selectedReport.formStatus)
              }
      ...
      {isReportFormModalOpen && (
        <Modal
          ...
          onClose={() => setIsReportFormModalOpen(false)}
        >
